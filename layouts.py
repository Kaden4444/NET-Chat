...
class DemoGUI(customtkinter.CTk):
    def __init__(self):
        ...
        # enter button
        self.send_message_button = customtkinter.CTkButton(master=self.main_frame, text="Send", fg_color=button_colour, border_width=1, border_color="black",text_color="black")
        self.send_message_button.grid(row=3, sticky='ne', padx=(0,10), pady=(10,0))
        self.send_message_button.configure(command= lambda:send_message(self.entry_textbox,self.chat_textbox))
        ...
        self.set_status_button.configure(command= lambda:change_status_to_available(self.set_status_button, self.chat_textbox))
...
def connect_to_server(button):
    ...
    try: 
        ...
        #main_server_ip="192.168.249.65"
        # main_server_ip = "192.168.56.1" #laptop
...
def change_status_to_available(button,textbox):
    ...
    request_waiter(textbox)
    waiterthread = threading.Thread(target= request_waiter(textbox))
    waiterthread.start()
...
def connect_to_client():
    ...
    if response.decode()[0:4] == "OKAY":
        ...
        client_connected_name = response.decode().split("-")[1]
    client.close()
    if connectedToPeer:
        return
...
def request_waiter(textbox):
    udpSocket = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
    udpSocket.bind(('', port_we_listen_on))
    global connected_to_client
    time.sleep(0.01)
    try:
        while connected_to_client == False: # Wait for communication from a client
            ...
            if message.decode()[0:17] == "REQ-COMMUNICATION":
                ...
                if reqbox:
                    ...
                    global other_client_name
                    global other_client_ip
                    global other_client_port

                    connected_to_client = True
                    other_client_name = message.decode().split("-")[2]
                    other_client_ip = clientAddress[0]
                    other_client_port = message.decode().split("-")[3]
                    udpSocket.close()
                # have to pass all the textbox through all these
                listener = threading.Thread(target=lambda: recieve_message(textbox), args=("Message Waiter 1",))
                listener.start()
