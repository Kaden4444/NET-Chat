...
def handle_client(client_socket, client_address):
    while True:
        ...
        if message.lower() == "disconnect":
            ...
            if request_client:
                prompt = f"Sending request to {target_name}"
                client_socket.send(prompt.encode('utf-8'))
                request_message = f"{clients[client_address]['name']} wants to start a chat. Accept? (yes/no)"
                request_client['socket'].send(request_message.encode('utf-8'))
                response = request_client['socket'].recv(1024).decode('utf-8').lower()
